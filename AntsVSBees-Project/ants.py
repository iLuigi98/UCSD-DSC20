    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                elif insect.can_contain(self.ant) != None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 5
        place_checked = self.place
        counter = 0

        while place_checked != hive:
            if counter >= self.min_range and counter <= self.max_range:
                if random_or_none(place_checked.bees) != None:
                    return random_or_none(place_checked.bees)
                else:
                    place_checked = place_checked.entrance
                    counter = counter + 1
            else:
                place_checked = place_checked.entrance
                counter = counter + 1
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        elif len(list(self.place.bees)) > 0:
            self.eat_bee(random.choice(list(self.place.bees)))
        # END Problem 10
